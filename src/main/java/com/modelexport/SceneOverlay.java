...
{
    private static final int CULL_CHUNK_BORDERS_RANGE = 64;
    private static final int LOCAL_TILE_SIZE = Perspective.LOCAL_TILE_SIZE;

    private final Client client;
    private final ModelExportConfig config;
    private final TooltipManager tooltipManager;

    @Inject
    public SceneOverlay(Client client, ModelExportConfig config, TooltipManager tooltipManager)
    {
...
//        }
            ...
            if (config.regionLines()) {
                renderLines(graphics, 64, 4, Color.green);
            }
...
//        }
    ...
    private void renderLines(Graphics2D graphics, int gridSize, int lineWidth, Color lineColour)
    {
        ...
        int startX = (wp.getX() - config.chunkSize() + gridSize - 1) / gridSize * gridSize;
        int startY = (wp.getY() - config.chunkSize() + gridSize - 1) / gridSize * gridSize;
        int endX = (wp.getX() + config.chunkSize()) / gridSize * gridSize;
        int endY = (wp.getY() + config.chunkSize()) / gridSize * gridSize;
        ...
        {
            LocalPoint lp1 = LocalPoint.fromWorld(client, x, wp.getY() - config.chunkSize());
            LocalPoint lp2 = LocalPoint.fromWorld(client, x, wp.getY() + config.chunkSize());
        ...
        {
            LocalPoint lp1 = LocalPoint.fromWorld(client, wp.getX() - config.chunkSize(), y);
            LocalPoint lp2 = LocalPoint.fromWorld(client, wp.getX() + config.chunkSize(), y);

            if (lp1 == null || lp2 == null)
            {
                log.info("Stuck Null 2");
                return;
            }
